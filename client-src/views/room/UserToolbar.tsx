...
export default function UserToolbar({ hat, frog, setFrog, setHat }: UserToolbarProps) {
...
function CustomizeFrogDialog({ dialogRef, frog, hat, onFrogComplete, onHatComplete }: CustomizeFrogDialogProps) {
  ...
  const currentHat = useMemo(() => _hatIndex !== null ? HATS[_hatIndex]! : hat, [_hatIndex, hat]);
  const currentFrog = useMemo(() => _frogIndex !== null ? FROGS[_frogIndex]! : frog, [_frogIndex, frog]);
  ...
  return (
    <dialog ref={dialogRef} className="bg-white absolute p-4">
      <div className="flex justify-between items-center">
        <button onClick={() => safeSetHatIndex(v => v - 1)} className="border group bg-green-400 hover:bg-green-600 p-2 rounded-full">
          <img src={EditPencil} className="group-hover:text-green-300" />
        </button>
      ...
      <div className="flex justify-between items-center">
        <button onClick={() => safeSetFrogIndex(v => v - 1)} className="border group bg-green-400 hover:bg-green-600 p-2 rounded-full">
          <img src={EditPencil} className="group-hover:text-green-300" />
        </button>
